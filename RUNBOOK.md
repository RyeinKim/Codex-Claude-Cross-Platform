...
| `BRIDGE_RESUME` | `0` | **옵트인, 루프 모드 전용**: `1`이면 각 CLI 세션을 resume해 전체 transcript 대신 상대의 최신 메시지(델타)만 전송. 기본 OFF (아래 레이트리밋 절 참고) |
...
- **`BRIDGE_RESUME=1`(옵트인, 루프 모드 전용)**은 매 턴 전체 transcript 대신 델타만 보내 캐시 할인을 노립니다. **실 CLI로 검증됨(2026-07-21)**: 두 계약 모두 성립하고 resume 턴이 대부분 캐시에서 청구됩니다 — claude `input_tokens=2`/`cache_read=15289`, codex `cached_input_tokens=29184`/`38890`. claude는 **1시간** 캐시라 `TURN_SLEEP`이 넉넉해도 할인이 유지되지만 codex는 창이 더 짧으니 `TURN_SLEEP`을 과하게 늘리지 마세요. 이는 턴당 입력 할인이지 전체 대화 절감의 A/B 측정은 아닙니다. Stage B는 여전히 전체 transcript를 보냅니다.
